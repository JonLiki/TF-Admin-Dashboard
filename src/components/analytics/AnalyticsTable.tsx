    ...
    return (
        <div className="overflow-x-auto rounded-xl border border-white/5 shadow-lg">
            <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-border/20 dark:divide-white/5 bg-ocean-deep">
                    {data.map((row, index) => (
                        <tr key={row.id} className={cn(
                            ...
                            <td className="px-6 py-4 text-right font-mono font-bold text-foreground/80 group-hover:text-foreground dark:text-slate-300 dark:group-hover:text-white tabular-nums">
                                {row.points}
                            </td>
                            <td className={cn("px-6 py-4 text-right font-mono tabular-nums text-foreground/80 group-hover:text-foreground dark:text-slate-300 dark:group-hover:text-white", activeMetric === 'weight' && "font-black text-tongan bg-tongan/5")}>
                                {row.weight.toFixed(1)} kg
                            </td>
                            <td className={cn("px-6 py-4 text-right font-mono tabular-nums text-foreground/80 group-hover:text-foreground dark:text-slate-300 dark:group-hover:text-white", activeMetric === 'km' && "font-black text-ocean-light bg-ocean-light/5")}>
                                {row.avgKm.toFixed(2)} km
                            </td>
                            <td className={cn("px-6 py-4 text-right font-mono tabular-nums text-foreground/80 group-hover:text-foreground dark:text-slate-300 dark:group-hover:text-white", activeMetric === 'lifestyle' && "font-black text-emerald-400 bg-emerald-500/5")}>
                                {row.avgLife.toFixed(1)}
                            </td>
                            <td className={cn("px-6 py-4 text-right font-mono tabular-nums text-foreground/80 group-hover:text-foreground dark:text-slate-300 dark:group-hover:text-white", activeMetric === 'attendance' && "font-black text-ocean bg-ocean/5")}>
